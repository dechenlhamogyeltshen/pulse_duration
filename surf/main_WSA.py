...
def spheroidal(onecme,model):
    ...
    # Find the arrival speed witsurfIN 1 day of arrival
...
for _, onecme in crlist.iterrows():
    ...
    vr_in = surfIN.map_v_boundary_inwards(v_wsa, 30*u.solRad, rmin)
    
    cr, cr_lon_init = surfIN.datetime2surfinputs(date)
    
    model = surf.SURF(v_boundary=vr_in, cr_num=cr, cr_lon_init = cr_lon_init,simtime = simtime, r_min=rmin, r_max=rmax, dt_scale=dt_scale, latitude=0*u.deg, frame = 'synodic', track_cmes = True, lon_out = 0*u.rad)
