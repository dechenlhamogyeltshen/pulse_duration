...
class ConeCME:
    ...
    def compute_arrival_at_location(self, longitude, radius):
        ...
        if not is_scalar:
            ...
            if not match_len:
                raise ValueError('longitude and radius must be single values or be arrays of length equal to the number of time steps')

        if is_scalar == 1: # real one
        #if is_scalar == 0:
...
def _setup_dirs_():
    ...
    dirs = {'ephemeris': os.path.join(cwd, 'data', 'ephemeris', 'ephemeris.hdf5'),
            ...
            'input': os.path.join(cwd, 'data', 'input'),
            'output': os.path.join(cwd, 'data', 'output'),
